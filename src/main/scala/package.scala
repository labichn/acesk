...
package object acesk {
  ...
  def eval(e: Expression): Set[State] = {
    val ce0 = Closure(e, MtEnv)
    val s0 = MtStore
    val k0 = MtKon
    avalCesk(ce0, s0, k0, Set((ce0, s0, k0)), false)
  }

  private def printDebug(ce: Closure, s: Store[_, _], k: Kontinuation) {
    println("=========== DEBUG ===========")
    println("<M, ρ>: "+ce)
    println("     σ: "+s)
    println("     κ: "+k)
  ...
  private def collectGarbage[T <: Location](grays: List[T],
                                            blacks: List[T],
                                            whites: List[T],
                                            s: Store[T, _]): List[T] =
   (grays, blacks, whites) match {
     case (gs, bs, w::ws) if gs.contains(w) =>
       val set = s(w)
       if (set.size > 0) {
         val locs = set.foldLeft[List[T]](Nil)((a, n) => n.ll:::a)
  ...
  type State = (Closure, PosStore, Kontinuation)

  private def avalCesk(ce: Closure, s: PosStore, k: Kontinuation, ss: Set[State], d: Boolean = false): Set[State] = {
    def evalCesk(ce1: Closure, s1: PosStore, k1: Kontinuation, ss: Set[State] = ss): Set[State] = {
      val state = (ce1, s1, k1)
    ...
    (ce, bs.foldLeft[PosStore](MtStore)((a, n) => ConsStore(n, s(n), a)), k) match {
      ...
      case (Closure(Num, e1), s, If(e, t, el, l)) =>
        def next(exp: Expression) =
          s(l).foldLeft[Set[State]](ss)((a, n) => n match {
            case k: Kontinuation => evalCesk(Closure(exp, e), s, k, a)
          })
        next(t)++next(el)
      case (Closure(Con(0), e1), s, If(e, t, el, l)) =>
        s(l).foldLeft[Set[State]](ss)((a, n) => n match {
          case k: Kontinuation => evalCesk(Closure(t, e), s, k, a)
          case _ => a
        })
      case (Closure(v: Value, e1), s, If(e, t, el, l)) =>
        s(l).foldLeft[Set[State]](ss)((a, n) => n match {
          case k: Kontinuation => evalCesk(Closure(el, e), s, k, a)
          case _ => a
        })
      ...
      case (Closure(v: Value, e), s, Fn(Closure(Fun(x, m), e1), l)) => // cesk3
        val l1 = PosLoc(ce.pos)
        s(l).foldLeft[Set[State]](ss)((a, n) => n match {
          case k: Kontinuation => evalCesk(Closure(m, e1.bind(x, l1)), s.bind(l1, Closure(v, e)), k, a)
          case _ => a
        })
      ...
      case (Closure(v: Value, e), s, Op(o, vcs, Nil, l)) => // cesk5
        s(l).foldLeft[Set[State]](ss)((a, n) => n match {
          case k: Kontinuation => evalCesk(Closure(reduce(o, (Closure(v, e)::vcs).reverse), MtEnv), s, k, a)
          case _ => a
        })
      case (Closure(v: Value, e), s, Op(o, vcs, c::cs, l)) => // cesk6
        evalCesk(c, s, Op(o, Closure(v, e)::vcs, cs, l))
      case (Closure(v: Var, e), s, k) =>  // cesk7
        s(e(v)).foldLeft[Set[State]](ss)((a, n) =>
          n match {
            case ce1: Closure => evalCesk(ce1, s, k, a)
            case _ => a
          })
      ...
      case (Closure(v: Value, e), s, St(l, l1)) => // cesk9
        s(l).foldLeft[Set[State]](ss)((a, n) =>
          n match {
            case ce1: Closure => s(l1).foldLeft[Set[State]](a)((a1, n1) =>
              n1 match {
                case k: Kontinuation => evalCesk(ce1, s.rebind(l, ce), k, a1)
                case _ => a1
              })
            case _ => a
          })
      case (Closure(Letrec((x, m)::xms, n), e), s, k) => // al3
        ...
        val e1 = e.bind(x, l)
        val l1 = PosLoc(ce.pos)
        evalCesk(Closure(m, e1), s.alloc(l).bind(l1, k), Lr(x::xms.map(_._1), Nil, xms.map(_._2), e1, n, l1))
      ...
      case (Closure(v: Value, e), s, Lr(x::Nil, vvs, Nil, e1, body, l)) =>
        ...
        s(l).foldLeft[Set[State]](ss)((a, n) => n match {
          case k: Kontinuation => evalCesk(Closure(body, e1), s1, k, a)
          case _ => a
        })
      case (Closure(v: Value, e), s, MtKon) => ss + ((ce,s,k))
      case _ =>
